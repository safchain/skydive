...
func (o *OnDemandProbeClient) OnWSJSONMessage(c shttp.WSSpeaker, m shttp.WSJSONMessage) {
	...
	case "CaptureStartReply":
		...
		o.wsServer.BroadcastMessage(shttp.NewWSJSONMessage(ondemand.NotificationNamespace, "CaptureNodeUpdated", query.Capture.UUID))
...
func (o *OnDemandProbeClient) OnNodeDeleted(n *graph.Node) {
	...
	if uuid, ok := o.registeredNodes[string(n.ID)]; ok {
		o.wsServer.BroadcastMessage(shttp.NewWSJSONMessage(ondemand.NotificationNamespace, "CaptureNodeUpdated", uuid))
	}
	o.RUnlock()
}
...
func (o *OnDemandProbeClient) onAPIWatcherEvent(action string, id string, resource api.Resource) {
	...
	case "init", "create", "set", "update":
		o.wsServer.BroadcastMessage(shttp.NewWSJSONMessage(ondemand.NotificationNamespace, "CaptureAdded", capture))
		o.onCaptureAdded(capture)
	case "expire", "delete":
		o.wsServer.BroadcastMessage(shttp.NewWSJSONMessage(ondemand.NotificationNamespace, "CaptureDeleted", capture))
...
func NewOnDemandProbeClient(g *graph.Graph, ch *api.CaptureAPIHandler, w *shttp.WSJSONMessageServer, etcdClient *etcd.EtcdClient) *OnDemandProbeClient {
	resources := ch.Index()
	...
	w.AddJSONMessageHandler(o, []string{ondemand.Namespace})
