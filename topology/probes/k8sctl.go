...
func (k8s *K8SCtlProbe) DeleteNSEdges() {
	...
	for _,edge := range edges{
		...
		app,_ := edge.GetFieldString("Application")
		linkType,_ := edge.GetFieldString("LinkType")
		if  app == "k8s" && linkType == "k8s policy"{
			k8s.Graph.Lock()
			k8s.Graph.DelEdge(edge)
			k8s.Graph.Unlock()
			from,_ := edge.GetFieldString("From")
			to,_ := edge.GetFieldString("To")
			netpolicy,_ := edge.GetFieldString("NetworkPolicy")
...
func (k8s *K8SCtlProbe) MessageFromUpdater(msgType string, obj interface{}) {
	...
	case "NodeAdded":
		...
		nodeName,_ := n.GetFieldString("Name")
		nodeID := n.ID
